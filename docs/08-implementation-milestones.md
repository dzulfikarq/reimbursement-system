...
| M2 | Master Data | departments/categories/users CRUD (admin), validations, search/filter/sort/pagination on all three lists | Admin manages masters via API with proper status codes |
...
| M5 | Dashboards & Audit | summary/trend/breakdown endpoints, Redis caching, budget usage, audit log writes | Dashboard reflects workflow actions; second load hits cache |
